...
def print_list(items):
    ...
    #loop through list of items and print each of them
    for item in items:
        print item
...
def long_words(words):
    ...
    #create a new_list that contains the results of the list comprehension
    #check to see if the the length of the word is great than 4. If it is, append it to the new_list
...
def n_long_words(words, n):
    ...
    new_list = [word for word in words if len(word) > int(n)]
    return new_list
...
def halvesies(numbers):
    ...
    new_list = [float(number)/2 for number in numbers]
    return new_list
...
def average(numbers):
    ...
    if not numbers:
        ...
        average_of_numbers = float(sum(numbers))/len(numbers)
...
def reverse_list(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    reversed_list = [item for item in items[-1::-1]]
...
def duplicates(items):
    ...
    if not items:
        ...
        duplicates_list = set([item for item in items if items.count(item) > 1])
        return sorted(list(duplicates_list))
